...
export default function MainPageSettings() {
  ...
  useEffect(() => {
    const fetchMainPageSettings = async () => {
      try {
        ...
        console.error('메인 페이지 설정 가져오기 오류:', error);
        setError('메인 페이지 설정을 불러오는 중 오류가 발생했습니다.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('메인 페이지 설정 저장 오류:', error);
      setError('메인 페이지 설정 저장 중 오류가 발생했습니다.');
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <AdminLayout title="메인 페이지 설정 - 위즈포레 관리자">
...
// 관리자 페이지 레이아웃 적용
MainPageSettings.getLayout = (page) => page;
