...
export default function AfterSchoolProgramsPage() {
  ...
  useEffect(() => {
    // 데이터에서 방과 후 프로그램 카테고리에 해당하는 프로그램만 필터링
    const filtered = programs.filter(program => program.category === '방과 후 프로그램');
    setAfterSchoolPrograms(filtered);
  }, []);
    ...
    <div className={styles.container}>
      <header className={styles.header}>
        <div className={styles.breadcrumb}>
          <Link href="/admin/programs">프로그램 안내</Link> &gt; 방과 후 프로그램
        </div>
        <h1>방과 후 프로그램 관리</h1>
        <p>위즈포레 사회서비스센터에서 제공하는 방과 후 프로그램을 관리합니다.</p>
      </header>
...
AfterSchoolProgramsPage.getLayout = function getLayout(page) {
  return (
    <AdminLayout title="방과 후 프로그램 관리 - 위즈포레 사회서비스센터">
      {page}
    </AdminLayout>
  );
};