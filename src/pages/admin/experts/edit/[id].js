...
export default function EditExpert() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 전문가 정보 가져오기
        const expertDoc = await getDoc(doc(db, '전문가', id));
  ...
  if (loading) {
    return (
      <AdminLayout title="전문가 정보 수정 - 위즈포레 관리자">
...
// 관리자 페이지 레이아웃 적용
EditExpert.getLayout = (page) => page;
