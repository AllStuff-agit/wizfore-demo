...
export default function TherapyServices() {
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      ...
      try {
        // 치료상담 카테고리의 데이터 가져오기
        const serviceData = await getServiceByCategory("치료상담");
        ...
        if (serviceData && serviceData.programs) {
          ...
          setAddDataMessage({ 
            type: 'info', 
            text: 'Firestore에 치료·상담 서비스 데이터가 없습니다. "기존 치료·상담 데이터 추가" 버튼을 클릭하여 데이터를 추가해 주세요.' 
          });
        }
      } catch (error) {
  ...
  const addDataToFirestore = async () => {
    try {
      ...
      // 이미 데이터가 있는지 확인
      const dataExists = await checkServiceExists("치료상담");
      ...
      // 서비스 데이터 추가
      await setServiceData("치료상담", serviceDefaultData.치료상담);
      ...
      // 업데이트된 데이터 다시 로드
      const updatedServiceData = await getServiceByCategory("치료상담");
          ...
          <div className={styles.programGrid}>
            {filteredPrograms.map((program) => (
              <div key={program.id} className={styles.programCard}>
                ...
                <p className={styles.programCategory}>치료·상담</p>
