...
export default function PendingInquiriesPage() {
  ...
  useEffect(() => {
    const fetchPendingInquiries = async () => {
      try {
        ...
        // Firestore에서 대기중 문의 데이터만 가져오기
        const inquiriesRef = collection(db, 'inquiries');
        const inquiriesQuery = query(
          inquiriesRef,
          where('isResponded', '==', false), // 대기중 문의만 필터링
  ...
  if (loading) {
    return (
      <AdminLayout title="대기중 문의 - 위즈포레 관리자">
    ...
    <AdminLayout title="대기중 문의 - 위즈포레 관리자">
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageDescription}>
          답변이 필요한 문의 사항을 확인하고 처리합니다. 각 문의에 답변을 등록하면 '처리완료' 상태로 변경됩니다.
        </p>
      </div>
      ...
      <div className={styles.tabs}>
        ...
        <Link href="/admin/inquiries/pending" className={`${styles.tab} ${styles.tabActive}`}>
          대기중 문의
        </Link>
        <Link href="/admin/inquiries/completed" className={styles.tab}>
          처리완료 문의
        </Link>
      </div>
                ...
                <tr key={inquiry.id}>
                  ...
                  <td>
                    <div className={styles.actionCell}>
                      <Link href={`/admin/inquiries/detail?id=${inquiry.id}`} className={styles.viewButton} title="답변하기">
...
// 관리자 페이지 레이아웃 적용
PendingInquiriesPage.getLayout = (page) => page;